...
Debug_mode = 3 # 1-> normal  // 2 -> add WT  // 3 -> set same WT (MLC -> 3760 // SLC -> 8000 )
more_wt = 10
same_wt = 6000
        ...
        for index2 in date_class:
            ...
            if Debug_mode == 1: 
                ...
                #print('Pttrn:',index2[4][2:9],'de',index2[5][0:3],'WT:', index2[9])
                    ...
                    TAP_WT = STF_WT
                    if index2[5][0:3] == 'STF':      
                        #+ SBFT_WAITTIME MAIN PAT g5025286V1928267D_Iw_EA041T1s_gnm0t1p06adeS_xxxlxxv_a080818bax202_EA2PTET0SC07x1ue_x00_exe_fpu_cascade_no_ps_denorm_1209010:STF STF_TEST_WAIT_END -10 OPCODE_OPERAND IT_LOOPSET 2000 # ORG 8000 SAVE 6760 MIN 1120
                        

                        print('+ SBFT_WAITTIME MAIN PAT'+  +' g5025286V' + index2[4][2:9] + 'D_Iw_EA041T1s_gnm0t1p06adeS_xxxlxxv_a080818bax202_EA2PTET0SC07x1ue_x00_exe_fpu_cascade_no_ps_denorm_1209010:STF' + ' STF_TEST_WAIT_END -10 OPCODE_OPERAND IT_LOOPSET ' +  index2[4][2:9]  )
                    ...
                    if index2[5][0:3] == 'TAP':
                        #+ SBFT_WAITTIME MAIN PAT g5025286V1928267D_Iw_EA041T1s_gnm0t1p06adeS_xxxlxxv_a080818bax202_EA2PTET0SC07x1ue_x00_exe_fpu_cascade_no_ps_denorm_1209010:TAP TAP_TEST_WAIT_END -5 OPCODE_OPERAND IT_LOOPSET 500 # ORG 2000 SAVE 1690 MIN 280                                   
    ...
    for index1 in pttrn_list:
        ...
        if Debug_mode == 3: 
            if index1 == index1: # Compare Class VID agains VID PLIST if are equals get data from Class
                ...
                TAP_WT = STF_WT//4
                #if index2[5][0:3] == 'STF':                                         
                print('+ SBFT_WAITTIME MAIN PAT' +' g5025286V' + index1 + 'D_Iw_EA041T1s_gnm0t1p06adeS_xxxlxxv_a080818bax202_EA2PTET0SC07x1ue_x00_exe_fpu_cascade_no_ps_denorm_1209010:STF' + ' STF_TEST_WAIT_END -10 OPCODE_OPERAND IT_LOOPSET ' +  str(STF_WT) )
                #if index2[5][0:3] == 'TAP':
